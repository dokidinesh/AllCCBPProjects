...
class App extends Component {
  ...
  onClickImage = imageId => {
    const {referenceImage} = this.state
    const randomValue = Math.floor(Math.random() * imagesList.length)
    const newReferenceImage = imagesList[randomValue]
    if (imageId === referenceImage.id) {
      this.setState(prevState => ({
        score: prevState.score + 1,
        referenceImage: newReferenceImage,
      }))
    } else if (imageId !== referenceImage.id) {
      clearInterval(this.uniqueId)
      this.setState({renderResultView: true})
    }
  }
  ...
  renderPlayingView = () => {
    const {activeTab, referenceImage} = this.state
    ...
    return (
      <div className="playing-view">
        ...
        <ul className="thumbnails-container">
          {activeTabImagesList.map(eachObject => (
            <ThumbnailImage
  ...
  render() {
    const {score, renderResultView, time} = this.state

    return (
      <div className="app-container">
        <Header score={score} time={time} />
        {time === 0 || renderResultView === true
