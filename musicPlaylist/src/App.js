...
class App extends Component {
  ...
  deleteSong = uniqueNo => {
    ...
    const filteredSongsList = songsList.filter(
      eachItem => eachItem.id !== uniqueNo,
    )

    this.setState({songsList: filteredSongsList})
  }

  renderFilteredSongs = () => {
    const {searchInput, songsList} = this.state

    const resultedSongsList = songsList.filter(eachSong =>
      eachSong.name.toLowerCase().includes(searchInput.toLowerCase()),
    )

    if (resultedSongsList.length !== 0) {
      return (
        <ul className="songs-list">
          {resultedSongsList.map(eachItem => (
            <SongCard
              key={eachItem.id}
      ...
      <div className="app-container">
        ...
        <div className="playlist-container">
          <div className="search-container">
            <h1 className="playlist-title">Songs Playlist</h1>
